...
All commands run through `uv run signac-deps ...` (or `signac-deps` if installed in
...
```bash
uv run signac-deps validate pipeline.toml
```
...
uv run signac-deps materialize pipeline.toml --project . --dry-run   # compute ids only
```
...
```bash
uv run signac-deps status pipeline.toml --project .
uv run signac-deps status pipeline.toml --project . --missing-only
```
...
```bash
uv run signac-deps collect-params pipeline.toml s3 --project . --format csv
uv run signac-deps collect-params pipeline.toml s3 --include-doc --format json
```
...
uv run signac-deps migrate-apply pipeline.toml s1 --project . --plan path/to/plan.json
uv run signac-deps migrate-apply pipeline.toml s1 --project . --dry-run
```
